            ...
            <div class="col-lg-12">
                ...
                <div class="col-md-5">
                    ...
                    <div class="form-group all">
                        ...
                        form_input('name', ($_POST['name'] ?? ($product ? $product->name : '')), 'class="form-control' . '" id="name" required="required"');
                        ?>
                    </div>
                    <div class="form-group all">
                        <?= lang('display_serial', 'serial') ?>
                        <?= form_input('display_serial', ($_POST['display_serial'] ?? ($product ? $product->display_serial : '')), 'class="form-control' . '" id="display_serial"'); ?>
                    </div>
                    <div class="form-group all">
                    ...
                    <div class="form-group all">
                        <input name="is_packed" type="checkbox" class="checkbox" id="is_packed" value="1" <?= isset($_POST['is_packed']) ? 'checked="checked"' : '' ?>/>
                        <label for="is_packed" class="padding05"><?= lang('is_packed') ?></label>
                    </div>
                    <div id="packed_product_area" style="display:none;">
                    <div class="form-group all">
                        <?= lang('pack_piece', 'pack_piece'); ?>
                        <?= form_input('pack_piece', set_value('pack_piece'), 'class="form-control tip" id="pack_piece"'); ?>
                    </div>
                    <div class="form-group">
                        <?= lang('packed_product', 'packed_product') ?>
                        <?php
                        $prod[''] = lang('select') . ' ' . lang('packed_product');
                        foreach ($products as $singproduct) {
                            $prod[$singproduct->id] = $singproduct->name;
                        }
                        echo form_dropdown('packed_product', $prod, ($_POST['packed_product'] ?? ''), 'class="form-control select" id="packed_product" style="width:100%"')
                        ?>
                    </div>
                    </div>
                    ...
                    <?php if ($Settings->tax1) {
                        ...
                        <div class="form-group all">
                            ...
                            echo form_dropdown('tax_rate', $tr, ($_POST['tax_rate'] ?? ($product ? $product->tax_rate : '')), 'class="form-control select" id="tax_rate" required="required" style="width:100%"')
                            ?>
                    ...
                    <div class="form-group all">
                        <?= lang('select_printer', 'select_printer') ?>
                        <select name="printer_selection" class="select2 form-control">
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        <?= isset($_POST['#is_packed']) ? '$("#is_packed").iCheck("check");' : '' ?>
        $('#is_packed').on('ifChecked', function (e) {
            $('#packed_product_area').slideDown();
        });
        $('#is_packed').on('ifUnchecked', function (e) {
