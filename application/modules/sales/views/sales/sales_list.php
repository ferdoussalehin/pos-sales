...
<div class="panel panel-default">
  	...
  	<div class="panel-heading">
		<div class="row">
			<div class="col-md-6">
				<h3><i class="fa-fw fa fa-barcode border-right"></i> <?php echo " Sale List" ?> </h3>
			</div>
			<div class="col-md-6">
				<div class="box-icon d-flex flex-row float-right"> 
						<div class="px-2 border-left">
							<?php
								echo  anchor("sales/add", '<i class="fa fa-plus-circle"></i>', array('class'=>'', 'title'=>'Add Sale'));
							?>
						</div>
...
$(document).ready(function(){
	...
	function fill_datatable(customf = ''){
	    ...
	    var customerDataTable = $(".datatable_display").DataTable({
	      ...
	      ajax:{
	          url:'<?php echo base_url(); ?>'+"sales/sale/getSales",
	          type: "POST",
	          data:{
	            customf : customf
	          }
	...
	$('body').on('click', '.sales_link td:not(:first-child, :nth-child(9), :last-child)', function () {
        
		var id = $(this).parent('.sales_link').attr('id');
		$.ajax({
            type:"get",
            url: site.base_url + 'sales/sale/modal_view/',
            data: { id: id },
            dataType: 'html',
	...
	$(document).on('click', '.add_payment', function(){
		...
		$.ajax({
			url: site.base_url + 'sales/sale/add_payment/',
			type: 'GET',
			dataType: 'html',
			data: {id: id},
			success: function(response) {
	...
	$(document).on('click', '.view_payment', function(){
		const id = $(this).data('id');
		$.ajax({
			url: site.base_url + 'sales/sale/view_payment/',
			type: 'GET',
			dataType: 'html',
			data: {id: id},
			success: function(response) {
