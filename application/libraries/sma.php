...
class Sma
{
    ...
    public function tlv_code($generatedString)
    {
        $sq = '';
        $type = '';
        $size = '';
        ...
        $config = ['data' => $generatedString, 'size' => $size, 'level' => $level, 'savename' => $file_name];
        $this->qr->generate($config);
        $imagedata = file_get_contents($file_name);

        return "<img src='data:image/png;base64," . base64_encode($imagedata) . "' alt='{$generatedString}' class='qrimg' />";
    }
}
